...
Page({
	...
	onLoad: function(options) {
		...
		if (options.sId) {
			var time = options.time.substring(0, 4) + '年' + options.time.substring(5, 7) + '月' + options.time.substring(8, 10) +
				'日'
			that.setData({
	...
	submit() {
		var that = this,
			date = this.data.date.substring(0, 4) + '/' + this.data.date.substring(5, 7) + '/' + this.data.date.substring(8,
				10),
